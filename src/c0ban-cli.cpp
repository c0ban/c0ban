...
static bool AppInitRPC(int argc, char* argv[])
{
    ...
    if (argc<2 || mapArgs.count("-?") || mapArgs.count("-h") || mapArgs.count("-help") || mapArgs.count("-version")) {
    	/* Modified by Qianren Zhang  Dec. 8, 2016 */
        std::string strUsage = strprintf(_("%s RPC client version: "), _(SOFTWARE_NAME)) + " " + FormatFullVersion() + "\n";
        if (!mapArgs.count("-version")) {
            strUsage += "\n" + _("Usage:") + "\n" +
                  "  coban-cli [options] <command> [params]  " + strprintf(_("Send command to %s"), _(SOFTWARE_NAME)) + "\n" +
                  "  coban-cli [options] help                " + _("List commands") + "\n" +
                  "  coban-cli [options] help <command>      " + _("Get help for a command") + "\n";
