...
public:
    ...
    virtual bool SignTransaction(CMutableTransaction& tx, const std::map<COutPoint, Coin>& coins, SigHashType& sigHashTypeIn, std::map<int, std::string>& input_errors) const { return false; }
    /** Sign a message with the given script */
    virtual SigningResult SignMessage(const std::string& message, const PKHash& pkhash, std::string& str_sig) const { return SigningResult::SIGNING_FAILED; };
    /** Adds script and derivation path information to a PSBT, and optionally signs it. */
    virtual TransactionError FillPSBT(PartiallySignedTransaction& psbt, SigHashType sigHashType = SigHashType().withForkId() /* SIGHASH_ALL */, bool sign = true, bool bip32derivs = false) const { return TransactionError::INVALID_PSBT; }
...
public:
    ...
    bool SignTransaction(CMutableTransaction& tx, const std::map<COutPoint, Coin>& coins, SigHashType& sigHashTypeIn, std::map<int, std::string>& input_errors) const override;
    SigningResult SignMessage(const std::string& message, const PKHash& pkhash, std::string& str_sig) const override;
    TransactionError FillPSBT(PartiallySignedTransaction& psbt, SigHashType sigHashType = SigHashType().withForkId() /* SIGHASH_ALL */, bool sign = true, bool bip32derivs = false) const override;
