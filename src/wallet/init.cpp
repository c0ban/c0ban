...
{
    gArgs.AddArg("-addresstype", strprintf("What type of addresses to use (\"legacy\" only. Do not with \"p2sh-segwit\", \"bech32\", default: \"%s\")", FormatOutputType(DEFAULT_ADDRESS_TYPE)), ArgsManager::ALLOW_ANY, OptionsCategory::WALLET);
    ...
    gArgs.AddArg("-changetype", "What type of change to use (\"legacy\" only. Do not with \"p2sh-segwit\", or \"bech32\"). Default is same as -addresstype, except when -addresstype=p2sh-segwit a native segwit output is used when sending to a native segwit address)", ArgsManager::ALLOW_ANY, OptionsCategory::WALLET);
