...
bool CNode::ReceiveMsgBytes(const char *pch, unsigned int nBytes)
{
    while (nBytes > 0) {
        ...
        if (!msg.in_data){
            ...
            //LogPrintf("MessageCommand=%s\n", msg.hdr.GetCommand());
            //if ( ( msg.hdr.GetCommand() != "ping" ) && ( msg.hdr.GetCommand()!= "pong" ) )
            //	LogPrintf("MessageCommand=%s nBytes=%u handled=%d\n", msg.hdr.GetCommand(), nBytes, handled);
        }
        else{
            handled = msg.readData(pch, nBytes);
            //LogPrintf("MessageData=%s\n",pch);
...
static void AcceptConnection(const ListenSocket& hListenSocket) {
    ...
    LogPrintf("net: connection from %s accepted\n", addr.ToString());
...
void ThreadSocketHandler()
{
    ...
    LogPrintf("Enter ThreadSocketHandle \n");
    while (true)
    {
...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        std::string strDesc = "c0ban " + FormatFullVersion();
    ...
    if (mapArgs.count("-connect") && mapMultiArgs["-connect"].size() > 0)
    {
        for (int64_t nLoop = 0;; nLoop++)
        {
            ...
            BOOST_FOREACH(const std::string& strAddr, mapMultiArgs["-connect"])
            {
                ...
                //LogPrintf("strAddr=%s, c_str=%s\n", strAddr, strAddr.c_str() );
    ...
    while (true)
    {
        ...
        if (addrman.size() == 0 && (GetTime() - nStart > 60)) {
            ...
            if (!done) {
                ...
                /* Modified by Qianren Zhang 	Nov. 1st, 2016 */
                addrman.Add(convertSeed6(Params().FixedSeeds()), local);
                LogPrintf("Adding fixed seed nodes after addman\n");
                done = true;
            }
        }
