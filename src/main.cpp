...
CAmount GetBlockSubsidy(int nHeight, const Consensus::Params& consensusParams)
{
    ...
    //LogPrintf("\n\n\n\n Going to GetBlockSubsidy  nHeight=%d \n\n", nHeight );
...
bool IsInitialBlockDownload()
{
    ...
    if (fImporting || fReindex)
        return true;
    /* Added by Qianren Zhang 		May 24, 2017 */
    ...
    //LogPrintf("IsInitialBlockDownloads: after Check check point \n");
    bool state = (chainActive.Height() < pindexBestHeader->nHeight - 24 * 6 ||
            std::max(chainActive.Tip()->GetBlockTime(), pindexBestHeader->GetBlockTime()) < GetTime() - nMaxTipAge);

    //LogPrintf("IsInitialBlockDownloads: state=%d \n", state );
    if (!state)
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    LogPrintf("bench:  - Sanity checks: %.2fms [%.2fs]\n", 0.001 * (nTime1 - nTimeStart), nTimeCheck * 0.000001);
    ...
    //LogPrintf("  ConenctBlock    height=%d, blockReward=%d,  Valueout=%d", pindex->nHeight, blockReward, block.vtx[0].GetValueOut());
...
static bool AcceptBlock(const CBlock& block, CValidationState& state, const CChainParams& chainparams, CBlockIndex** ppindex, bool fRequested, const CDiskBlockPos* dbp, bool* fNewBlock)
{
    ...
    try {
        ...
        //LogPrintf("AcceptBlock: blockPos.nPos=%d, dbp=%s, nHeight=%d\n", blockPos.nPos, dbp, nHeight );
        //LogPrintf("AcceptBlock: nPos=%d, dbp=%s, nHeight=%d \n", blockPos.nPos, dbp, nHeight);
        //LogPrintf("AcceptBlock: block= %s \n", block.ToString() );
        //LogPrintf("AcceptBlock: block= %s \n", block.ToString());
        if (dbp == NULL)
        ...
        if (!ReceivedBlockTransactions(block, state, pindex, blockPos))
        	...
        	//LogPrintf("AcceptBlock:  Block has been written to the disk\n");
    } catch (const std::runtime_error& e) {
...
bool TestBlockValidity(CValidationState& state, const CChainParams& chainparams, const CBlock& block, CBlockIndex* pindexPrev, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    //LogPrintf("After ContextualCheckBlockHeader()\n");
    if (!CheckBlock(block, state, chainparams.GetConsensus(), fCheckPOW, fCheckMerkleRoot))
        return error("%s: Consensus::CheckBlock: %s", __func__, FormatStateMessage(state));
    //LogPrintf("After CheckBlock()\n");
    if (!ContextualCheckBlock(block, state, chainparams.GetConsensus(), pindexPrev))
        return error("%s: Consensus::ContextualCheckBlock: %s", __func__, FormatStateMessage(state));
    //LogPrintf("After ContextualCheckBlock()\n");
    if (!ConnectBlock(block, state, &indexDummy, viewNew, chainparams, true))
        return false;
    //LogPrintf("After ConnectBlock()\n");
    assert(state.IsValid());
    //LogPrintf("After assert\n");
    return true;
}
...
std::string GetWarnings(const std::string& strFor)
{
    ...
    if (!CLIENT_VERSION_IS_RELEASE) {
        strStatusBar = "c0ban wallet software for managing c0ban coins";
        strGUI = _("c0ban wallet software for managing c0ban coins");
    }
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived, const CChainParams& chainparams)
{
    ...
    if (strCommand == NetMsgType::VERSION)
    {
        ...
        LogPrintf("receive version message: %s: version %d, blocks=%d, us=%s, peer=%d%s\n",
        		FormatSubVersion2(), pfrom->nVersion,
