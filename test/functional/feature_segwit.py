...
class SegWitTest(BitcoinTestFramework):
    ...
    def success_mine(self, node, txid, sign, redeem_script=""):
        send_to_witness(1, node, getutxo(txid), self.pubkey[0], False, Decimal("21999.998"), sign, redeem_script)
    ...
    def fail_accept(self, node, error_msg, txid, sign, redeem_script=""):
        assert_raises_rpc_error(-26, error_msg, send_to_witness, use_p2wsh=1, node=node, utxo=getutxo(txid), pubkey=self.pubkey[0], encode_p2sh=False, amount=Decimal("21999.998"), sign=sign, insert_redeem_script=redeem_script)
    ...
    def run_test(self):
        ...
        for i in range(5):
            for n in range(3):
                for v in range(2):
                    wit_ids[n][v].append(send_to_witness(v, self.nodes[0], find_spendable_utxo(self.nodes[0], 22000), self.pubkey[n], False, Decimal("21999.999")))
                    p2sh_ids[n][v].append(send_to_witness(v, self.nodes[0], find_spendable_utxo(self.nodes[0], 22000), self.pubkey[n], True, Decimal("21999.999")))

        self.nodes[0].generate(1)  # block 163
        self.sync_blocks()

        # Make sure all nodes recognize the transactions as theirs
        assert_equal(self.nodes[0].getbalance(), balance_presetup - 60 * 22000 + 20 * Decimal("21999.999") + 22000)
        assert_equal(self.nodes[1].getbalance(), 20 * Decimal("21999.999"))
        assert_equal(self.nodes[2].getbalance(), 20 * Decimal("21999.999"))
        ...
        txid1 = send_to_witness(1, self.nodes[0], find_spendable_utxo(self.nodes[0], 22000), self.pubkey[0], False, Decimal("21999.996"))
        ...
        tx.vout.append(CTxOut(int(21999.99 * COIN), CScript([OP_TRUE, OP_DROP] * 15 + [OP_TRUE])))
        ...
        tx.vout.append(CTxOut(int(21999.95 * COIN), CScript([OP_TRUE, OP_DROP] * 15 + [OP_TRUE])))  # Huge fee
        tx.calc_sha256()
    ...
    def mine_and_test_listunspent(self, script_list, ismine):
        utxo = find_spendable_utxo(self.nodes[0], 22000)
        tx = CTransaction()
