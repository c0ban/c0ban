...
class SegWitTest(BitcoinTestFramework):
    ...
    def fail_accept(self, node, error_msg, txid, sign, redeem_script=""):
        assert_raises_rpc_error(-26, error_msg, send_to_witness, 1, node, getutxo(txid), self.pubkey[0], False, Decimal("21999.998"), sign, redeem_script)
    ...
    def run_test(self):
        ...
        for i in range(5):
            for n in range(3):
                for v in range(2):
                    wit_ids[n][v].append(send_to_witness(v, self.nodes[0], find_unspent(self.nodes[0], 22000), self.pubkey[n], False, Decimal("21999.999")))
                    p2sh_ids[n][v].append(send_to_witness(v, self.nodes[0], find_unspent(self.nodes[0], 22000), self.pubkey[n], True, Decimal("21999.999")))
        ...
        txid1 = send_to_witness(1, self.nodes[0], find_unspent(self.nodes[0], 22000), self.pubkey[0], False, Decimal("21999.996"))
        ...
        tx.vout.append(CTxOut(int(21999.99*COIN), CScript([OP_TRUE])))
        ...
        tx.vout.append(CTxOut(int(21999.95*COIN), CScript([OP_TRUE]))) # Huge fee
        tx.calc_sha256()
    ...
    def mine_and_test_listunspent(self, script_list, ismine):
        utxo = find_unspent(self.nodes[0], 22000)
        tx = CTransaction()
