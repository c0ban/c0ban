...
class BIP66Test(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        block = create_block(int(tip, 16), create_coinbase(DERSIG_HEIGHT - 1), block_time, DERSIG_HEIGHT - 1)
        block.nVersion = 2
        ...
        tip = block.sha256
        block_time += 1
        block = create_block(tip, create_coinbase(DERSIG_HEIGHT), block_time, DERSIG_HEIGHT)
        block.nVersion = 2
        block.rehash()
        block.solve()
        node0.send_and_ping(msg_block(block))
